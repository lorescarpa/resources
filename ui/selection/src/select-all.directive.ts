 ...
 *
 * Must be used within a parent `BriSelectionDirective` directive. It toggles the selection states
 * of all the selection toggles connected with the `BriSelectionDirective` directive.
 * If the element implements `ControlValueAccessor`, e.g. `MatCheckbox`, the directive
 * automatically connects it with the select-all state provided by the `BriSelectionDirective` directive. If
 * not, use `checked$` to get the checked state, `indeterminate$` to get the indeterminate state,
 * and `toggle()` to change the selection state.
 */
@Directive({
  standalone: true,
  selector: '[briSelectAll]',
  exportAs: 'briSelectAll',
})
export class BriSelectAllDirective<T> implements OnDestroy, OnInit {
  /**
  ...
  constructor(
    ...
    private readonly _selection: BriSelectionDirective<T>,
    @Optional()
    @Self()
    ...
    this.indeterminate = _selection.changed.pipe(
  ...
  private _assertValidParentSelection() {
    if (!this._selection && isDevMode()) {
      throw Error('BriSelectAll: missing BriSelection in the parent');
    }

    if (!this._selection.multiple && isDevMode()) {
      throw Error(
        'BriSelectAll: BriSelection must have briSelectionMultiple set to true'
      );
    }
  }
