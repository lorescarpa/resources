 ...
 * calendar popup. For consistency, we always use BriDatepickerInputEvent instead.
 */
export class BriDatepickerInputEvent<D, S = unknown> {
  /** The new value for the target datepicker input. */
  value: D | null;

  constructor(
    /** Reference to the datepicker input component that emitted the event. */
    public target: BriDatepickerInputBase<S, D>,
...
/**
 * Partial representation of `BriFormField` that is used for backwards-compatibility
 * between the legacy and non-legacy variants.
 */
export interface _BriFormFieldPartial {
...
export abstract class BriDatepickerInputBase<
  S,
    ...
    Validator
{
  ...
  protected _model: BriDateSelectionModel<S, D> | undefined;
  ...
  @Output() readonly dateChange: EventEmitter<BriDatepickerInputEvent<D, S>> =
    new EventEmitter<BriDatepickerInputEvent<D, S>>();

  /** Emits when an `input` event is fired on this `<input>`. */
  @Output() readonly dateInput: EventEmitter<BriDatepickerInputEvent<D, S>> =
    new EventEmitter<BriDatepickerInputEvent<D, S>>();
  ...
  private _parseValidator: ValidatorFn = (): ValidationErrors | null => {
    return this._lastValueValid
      ? null
      : { briDatepickerParse: { text: this._elementRef.nativeElement.value } };
  };
  ...
  private _minValidator: ValidatorFn = (
    ...
    return !min ||
      ...
      ? null
      : { briDatepickerMin: { min: min, actual: controlValue } };
  };
  ...
  private _maxValidator: ValidatorFn = (
    ...
    return !max ||
      ...
      ? null
      : { briDatepickerMax: { max: max, actual: controlValue } };
  };
  ...
  _registerModel(model: BriDateSelectionModel<S, D>): void {
    this._model = model;
    ...
    this._valueChangesSubscription = this._model.selectionChanged.subscribe(
      (event) => {
        if (this._shouldHandleChangeEvent(event)) {
          ...
          this.dateInput.emit(
            new BriDatepickerInputEvent(this, this._elementRef.nativeElement)
          );
