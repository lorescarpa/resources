...
export type BriCalendarCellCssClasses =
  | string
  | string[]
  | Set<string>
  | { [key: string]: any };
...
export type BriCalendarCellClassFunction<D> = (
  date: D,
  view: 'month'
) => BriCalendarCellCssClasses;
 ...
 * @docs-private
 */
export class BriCalendarCell<D = any> {
  ...
  constructor(
    ...
    public cssClasses: BriCalendarCellCssClasses = {},
...
export interface BriCalendarUserEvent<D> {
  value: D;
  event: Event;
}
...
@Component({
  standalone: true,
  selector: '[bri-calendar-body]',
  templateUrl: './calendar-body.component.html',
  styleUrl: './calendar-body.component.scss',
  host: {
    class: 'bri-calendar-body',
  },
  exportAs: 'briCalendarBody',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [NgClass],
})
export class BriCalendarBodyComponent<D = any>
  implements OnChanges, OnDestroy, AfterViewChecked
{
  ...
  @Output() readonly selectedValueChange = new EventEmitter<
    BriCalendarUserEvent<number>
  >();

  /** Emits when the preview has changed as a result of a user action. */
  @Output() readonly previewChange = new EventEmitter<
    BriCalendarUserEvent<BriCalendarCell | null>
  >();
  ...
  @Output() readonly dragStarted = new EventEmitter<BriCalendarUserEvent<D>>();
   ...
   */
  _trackRow = (row: BriCalendarCell[]) => row;
  ...
  _cellClicked(cell: BriCalendarCell, event: MouseEvent): void {
  ...
  _emitActiveDateChange(cell: BriCalendarCell, event: FocusEvent): void {
    if (cell.enabled) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['cellAspectRatio'] || columnChanges || !this._cellPadding) {
      this._cellPadding = `${(35 * this.cellAspectRatio) / numCols}%`;
    }
  ...
  _isComparisonBridgeStart(value: number, rowIndex: number, colIndex: number) {
    ...
    let previousCell: BriCalendarCell | undefined =
  ...
  _isComparisonBridgeEnd(value: number, rowIndex: number, colIndex: number) {
    ...
    let nextCell: BriCalendarCell | undefined =
      this.rows[rowIndex][colIndex + 1];
  ...
  private _mouseupHandler = (event: Event) => {
    ...
    if (
      cellElement.closest('.bri-calendar-body') !==
  ...
  /** Finds the BriCalendarCell that corresponds to a DOM node. */
  private _getCellFromElement(element: HTMLElement): BriCalendarCell | null {
    const cell = getCellElement(element);

    if (cell) {
      const row = cell.getAttribute('data-bri-row');
      const col = cell.getAttribute('data-bri-col');
  ...
  private _id = `bri-calendar-body-${calendarBodyId++}`;
...
function getCellElement(element: HTMLElement): HTMLElement | null {
  ...
  return cell?.getAttribute('data-bri-row') != null ? cell : null;
}
