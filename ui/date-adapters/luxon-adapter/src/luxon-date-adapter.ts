...
import { DateAdapter, BRI_DATE_LOCALE } from '@wkp/bricks/core';
import {
...
export interface BriLuxonDateAdapterOptions {
  useUtc: boolean;
...
export const BRI_LUXON_DATE_ADAPTER_OPTIONS =
  new InjectionToken<BriLuxonDateAdapterOptions>(
    'BRI_LUXON_DATE_ADAPTER_OPTIONS',
    {
      providedIn: 'root',
      factory: BRI_LUXON_DATE_ADAPTER_OPTIONS_FACTORY,
    }
  );

/** @docs-private */
export function BRI_LUXON_DATE_ADAPTER_OPTIONS_FACTORY(): BriLuxonDateAdapterOptions {
  return {
    useUtc: false,
    firstDayOfWeek: 0,
...
export class LuxonDateAdapter extends DateAdapter<LuxonDateTime> {
  ...
  constructor(
    @Optional() @Inject(BRI_DATE_LOCALE) dateLocale: string,
    @Optional()
    @Inject(BRI_LUXON_DATE_ADAPTER_OPTIONS)
    options?: BriLuxonDateAdapterOptions
  ) {
    super();
  ...
  override getMonthsBetwweenDates(
    startDate: LuxonDateTime<boolean>,
    endDate: LuxonDateTime<boolean>
  ): { index: number; name: string }[] {
    const months: { [key: string]: { index: number; name: string } } = {};

    let current = startDate.startOf('month');
    while (current <= endDate && Object.keys(months).length <= 12) {
      const monthIndex = current.month - 1;
      const monthName = current.setLocale(this.locale).toFormat('MMMM');
